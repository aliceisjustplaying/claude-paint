# Amnesia round 4: paint at the easel
...
- Friedrich drew before he painted. The easel has drawing tools (see the
  guide): a pencil and chalk underdrawing you can sketch, look at, erase and
  paint into (`drawing_guide()`), a drawing hand that turns a few rough
  points into a hand-drawn outline or body (`outline{}`, `body_of{}`), a
  coordinate grid, probes and `show()` previews on the look, `easel try` for
  dry runs, depth masks from the world (`visible=`, `behind=`, shadows) and
  `easel edit N` to change an early chunk in place. Use whatever helps you
  draw by eye.
- Friedrich's pictures are full of tiny particular details, especially in
  foregrounds. Don't stop at broad passages; use 3200 crops (live:
  `look --crop ... --scale 3.2`).
...
2. notes/{NAME}.md, written continuously: the picture and why (what in
   ...
   it compares with what you'd expect from writing a program, and in
   particular **whether you could draw** (shapes, figures, rocks, plants) by
   eye or had to compute them; FRICTION: every
