...
fn main() {
    ...
    // the river: depth t = 0 at the horizon, 1 at the bottom edge
    let depth = move |y: f32| ((y - yh) / (h - yh)).clamp(0.0, 1.0);
    let rn = Fbm::new(seed + 2, 3, 90.0);
    let river_c = move |y: f32| {
        let t = depth(y);
        505.0 - 250.0 * t.powf(1.25) + 18.0 * (t * 5.0).sin() * t
    };
    let river_hw = move |y: f32| {
        let t = depth(y);
        2.0 + 205.0 * t.powf(1.08)
    };
    // the banks wander a little: a bank is never a ruled curve
    let bank_l = move |y: f32| river_c(y) - river_hw(y) + 7.0 * depth(y) * rn.get(y * 1.3, 1.0);
    let bank_r = move |y: f32| river_c(y) + river_hw(y) + 7.0 * depth(y) * rn.get(y * 1.3, 9.0);
    ...
    if o.stage("sky", &mut c, &mut rng) {
        c.work(&sky, &st.broad().color(sky_color).angle(sky_angle).medium(0.3).load(0.62).pressure(0.65, 0.9).coverage(4.0), o.seed * 100 + 10);
        ...
        for k in 0..22 {
            ...
            let col = if v < 0.3 { gradient(&[(0.0, hex("#7f8895")), (1.0, hex("#9ea1a8"))], v / 0.3, Mix::Pigment) } else { gradient(&[(0.0, hex("#b3aeab")), (1.0, hex("#d5c0b0"))], (v - 0.3) / 0.2, Mix::Pigment) };
            fil.reload(Paint::new(st.palette.mix(col).color, 0.4, 0.45), 0.7 * 0.5);
    ...
    if o.stage("fields", &mut c, &mut rng) {
        ...
        let snow_c = move |x: f32, y: f32| {
            ...
            gradient(&[(0.0, base), (1.0, shade)], 0.6, Mix::Pigment)
        };
    ...
    if o.stage("ice", &mut c, &mut rng) {
        ...
        let ice_c = move |x: f32, y: f32| {
            let t = depth(y);
            let base = gradient(&[(0.0, hex("#b9b6b9")), (0.4, hex("#a6a8ae")), (1.0, hex("#8f949c"))], t, Mix::Pigment);
            // the sun's light lies on the far ice as a warm sheen below it
            let sheen = (-((x - sx - 60.0 * t) / (40.0 + 90.0 * t)).powi(2)).exp() * (1.0 - t).powf(0.7) * 0.7;
            let lit = gradient(&[(0.0, base), (1.0, hex("#dcd0bf"))], sheen, Mix::Pigment);
            // patches of wind-blown snow over the ice
            let s = smoothstep(0.1, 0.45, isn.get(x * 0.5, y * 2.5));
            gradient(&[(0.0, lit), (1.0, hex("#d2cfcd"))], s * 0.8, Mix::Pigment)
        };
    ...
    // stand over the far)
    let willow_ts = [0.035f32, 0.06, 0.1, 0.155, 0.23, 0.33, 0.47, 0.66];
    let willows: Vec<(f32, f32, f32)> = willow_ts
        .iter()
        .enumerate()
        .map(|(i, &t)| {
            let y = yh + t * (h - yh) + (i as f32 * 1.7).sin() * 2.0 * t;
            let x = bank_r(y) + 10.0 + 75.0 * t + 12.0 * t * (i as f32 * 2.3).cos();
            let ht = 7.0 + 330.0 * t;
            (x, y, ht)
        })
        .collect();
    // the figure: on the bank path, between the second and third willows
    let (fx, fy, fh) = (812.0f32, yh + 0.45 * (h - yh), 62.0f32);

    if o.stage("banks", &mut c, &mut rng) {
        // the lip of the right bank: a rim of snow over a dark undercut edge
        // where the bank drops to the ice, and on the left a low bank of
        // snow with a darker margin of old reeds
        let mut edge = Held::new(Tool::round_sable(2.4), 60);
        let mut y = yh + 2.0;
        while y < h + 4.0 {
            let t = depth(y);
            let step = 1.5 + 9.0 * t;
            for side in [0, 1] {
                let x = if side == 0 { bank_l(y) } else { bank_r(y) };
                let ww = 0.5 + 3.5 * t;
                edge.tool.width = 1.0 + 4.0 * t;
                edge.reload(pile(&st, if side == 0 { "#77746f" } else { "#6a6560" }, 0.8, 0.5), 0.6);
                let dir = if side == 0 { -1.0 } else { 1.0 };
                let pts = vec![(x + dir * ww * 0.2, y), (x + dir * ww * 0.6 + rng.range(-0.5, 0.5), y + step * 0.5), (x + dir * ww, y + step * rng.range(0.9, 1.3))];
                c.drag(&mut edge, &Gesture::new(pts).pressure(rng.range(0.4, 0.8), rng.range(0.2, 0.5)).ramps(0.1, 0.3), None);
            }
            y += step * rng.range(0.7, 1.1);
        }
        c.dry();
        ...
        while y < h + 4.0 {
            ...
            sn.reload(pile(&st, "#d9d5ce", 0.9, 0.8), 0.7);
            let x = bank_r(y) + 2.0 + 4.0 * t;
            let l = 4.0 + 20.0 * t;
            if rng.f() < 0.75 {
                c.drag(&mut sn, &Gesture::new(vec![(x + l, y + 1.0), (x + l * 0.4, y + 0.3), (x - l * 0.05 * rng.f(), y)]).pressure(rng.range(0.4, 0.8), 0.2).ramps(0.1, 0.5), None);
            }
            y += (2.0 + 12.0 * t) * rng.range(0.7, 1.2);
        }
        c.dry();
    }
    ...
    if o.stage("shadows", &mut c, &mut rng) {
        ...
        // away from the light (to the right and toward us): thin lean
        // strokes of a cool blue-violet, fading as they go
        let mut sh = Held::new(Tool { lay: 0.6, ..Tool::filbert(6.0) }, 70);
        for &(x, y, ht) in &willows {
            let t = depth(y);
            sh.tool.width = 0.8 + 7.0 * t;
            let len = ht * 1.1;
            for k in 0..3 {
                sh.reload(pile(&st, "#9fa0ae", 0.45, 0.4), 0.5);
                let off = (k as f32 - 1.0) * sh.tool.width * 0.35;
                let pts = vec![(x, y + off * 0.3), (x + len * 0.45, y + len * 0.06 + off), (x + len, y + len * 0.13 + off)];
                c.drag(&mut sh, &Gesture::new(pts).pressure(0.55, 0.05).ramps(0.05, 0.8).swell(vec![1.0, 0.8, 1.1, 0.7]), Some(&snow_fields));
            }
        }
        c.dry();
    }
    ...
    if o.stage("details", &mut c, &mut rng) {
        ...
        for _ in 0..90 {
            // and the near right corner
            spots.push((rng.range(640.0, w + 10.0), rng.range(h * 0.86, h + 5.0)));
        }
        ...
        for (gx, gy) in spots {
            ...
            let n = (2.0 + 7.0 * t * rng.f()) as u32 + 1;
            for _ in 0..n {
                let x = gx + rng.range(-3.0, 3.0) * (0.3 + t);
                let hg = rng.range(2.0, 7.0) * (0.25 + 2.2 * t);
...
fn willow(c: &mut paint::Canvas, st: &Style, rng: &mut Rng, x: f32, y: f32, ht: f32, sun_x: f32, id: u64) {
    ...
    let rod_col = gradient(&[(0.0, hex("#5a3e2c")), (1.0, hex("#8a848c"))], far.powf(1.1), Mix::Pigment);
    ...
    for k in 0..strokes {
        ...
        let x0 = x + u * tw * 0.5 + rng.range(-0.1, 0.1) * tw;
        let bulge = rng.range(0.0, 0.25) * tw;
        let pts = vec![
            (x0 + u * tw * 0.08, y + 1.5 * s),
            (x0 + lean * trunk_h * 0.33 + u * bulge * 0.3, y - trunk_h * 0.33),
            (x0 + lean * trunk_h * 0.66 - u * bulge * 0.2, y - trunk_h * 0.7),
            (head.0 + u * tw * 0.75, head.1 + rng.range(-0.05, 0.05) * trunk_h),
        ];
    ...
    let n = (18.0 + 60.0 * s.min(1.6)) as usize;
    let mut rod = Held::new(Tool { point: 1.0, ..Tool::rigger((0.35 + 0.5 * s).min(1.2)) }, 902 + id * 17);
    let rod_h = ht - trunk_h;
    for k in 0..n {
        if k % 6 == 0 {
            let tint = rng.range(-0.15, 0.15);
            let col = gradient(&[(0.0, rod_col), (1.0, if tint > 0.0 { hex("#8a6a4a") } else { hex("#3e342e") })], tint.abs() * (1.0 - far), Mix::Pigment);
    ...
    let mut sn = Held::new(Tool { point: 0.5, ..Tool::round_sable((tw * 0.28).max(0.6)) }, 903 + id * 17);
    for _ in 0..(3.0 + 6.0 * s.min(1.5)) as usize {
        sn.reload(pile(st, "#e2ddd3", 0.95, 0.9), 0.7);
        let a = rng.range(-0.9, 0.9);
        let px = head.0 + a * tw * 0.8;
        let py = head.1 - tw * rng.range(0.2, 0.4);
        c.touch(&mut sn, &Touch::at(px, py).pressure(rng.range(0.4, 0.8)).drag(rng.range(0.2, 0.6) * tw, rng.range(-0.05, 0.05) * tw), None);
    }
    ...
    for k in 0..4 {
        dr.reload(pile(st, if k % 2 == 0 { "#d8d4cc" } else { "#c8c6c8" }, 0.92, 0.85), 0.7);
        let yy = y + rng.range(-0.4, 1.2) * s * 3.0;
        let side = if k % 2 == 0 { -1.0 } else { 1.0 };
        let x0 = x + side * tw * rng.range(0.9, 1.6);
...
fn man_from_behind(c: &mut paint::Canvas, st: &Style, rng: &mut Rng, x: f32, y: f32, ht: f32) {
    ...
    let mut fb = Held::new(Tool { lay: 0.9, ..Tool::filbert(5.5 * u) }, 92);
    let sh_y = y - 80.0 * u;
    let hem_y = y - 13.0 * u;
    for k in 0..9 {
        let v = (k as f32 + 0.5) / 9.0 * 2.0 - 1.0;
        fb.reload(if v < -0.6 { coat_lit } else { coat }, 0.8);
        let top = (x + v * 11.0 * u, sh_y + v.abs().powi(2) * 5.0 * u);
        let mid = (x + v * 12.5 * u + rng.range(-0.4, 0.4) * u, y - 45.0 * u);
        let bot = (x + v * 16.0 * u + rng.range(-0.8, 0.8) * u, hem_y + rng.range(-1.5, 1.5) * u);
        c.drag(&mut fb, &Gesture::new(vec![top, mid, bot]).pressure(0.85, 0.9).ramps(0.05, 0.1).shake(0.6), None);
    }
    ...
    c.drag(&mut st_b, &Gesture::new(vec![(x + 15.0 * u, y - 44.0 * u), (x + 19.0 * u, y - 20.0 * u), (x + 22.0 * u, y + 1.0 * u)]).pressure(0.8, 0.7).ramps(0.05, 0.1).orient(Orient::Along), None);
