-- easel session "barerow_L": a painting replayed chunk by chunk.
--   easel run notes/lab2/barerow_L.lua [--width 3200]
...
-- L: the middle and the far oak by selected drawn lines only, as the near one but smaller brushes, fewer
-- fine pieces (detail 0.35 middle, 0.25 far, thinned again by a clustered noise sized to the crown), and
-- every color pulled toward the air by the tree's distance. The finest pieces are finer than the rigger's
-- finest line, so they come out at its finest line (a hair) in a paler color.
function lines(t, h, detail, seed)
  ...
  local n, kept = 0, 0
  for _, bd in ipairs(bands) do
    for _, st in ipairs(t:wood_strokes{min=bd[1], max=bd[2], detail=detail}) do
      local i = st.limb
...
print("far", lines(far, HAZE.far, 0.25, 91))
print("middle", lines(mid, HAZE.mid, 0.35, 92))
...
--@ chunk 11 · clock 54546.810546875
