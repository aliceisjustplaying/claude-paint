...
fn main() {
    ...
    if o.stage("underpainting", &mut c, &mut rng) {
        ...
        let hd = st
            .broad()
            .color(move |x, y| if y > knoll(x) - 4.0 { hex("#3a2f25") } else { mix(hex("#4a4d56"), hex("#6d6d72"), smoothstep(near_crest(x) + 40.0, near_crest(x) + 140.0, y), Mix::Pigment) })
            .by_masstone()
    ...
    if o.stage("far range", &mut c, &mut rng) {
        ...
        let fcol = &fcol;
        let hd = st.body().color(fcol).angle(|x, y| form.fall(x, y)).angle_jitter(0.15).length(8.0, 26.0).coverage(3.0).medium(0.4).tool_width(5.0).clip(true).threshold(0.2);
        c.work(&sil, &hd, 201);
        let sp = Stipple::new(Tool::stippler(2.2)).mixed(pal, 0.5).color(fcol).coverage(|_, _| 2.0).pressure(0.45, 0.8).dips(18, 0.4, 0.5).clip(true);
    ...
    if o.stage("far mist", &mut c, &mut rng) {
        ...
        let m = Stipple::new(Tool::stippler(2.4)).mixed(pal, 0.7).color(move |x, y| mix(air(x, y), hex("#efe3c8"), 0.3, Mix::Light)).coverage(move |x, y| 2.8 * mist_at(x, y, foot(x), 38.0)).pressure(0.5, 0.85).dips(16, 0.3, 0.7).aim(false);
        c.stipple(&mm, &m, 211);
        c.dry();
    }
    ...
    if o.stage("mid range", &mut c, &mut rng) {
        ...
        c.work(&sil, &hd, 221);
        let sp = Stipple::new(Tool::stippler(2.0)).mixed(pal, 0.45).color(mcol).coverage(|_, _| 1.4).pressure(0.45, 0.8).dips(18, 0.4, 0.5).clip(true);
    ...
    if o.stage("near range", &mut c, &mut rng) {
        ...
        let ncol = &ncol;
        let hd = st.body().color(ncol).angle(|x, y| form.fall(x, y)).angle_jitter(0.15).length(7.0, 22.0).coverage(3.2).medium(0.3).tool_width(4.5).clip(true).threshold(0.2);
        c.work(&sil, &hd, 231);
        // the woods: short hatching, upright, "like a closely woven textile"
        let wm = Mask::from_fn(f, move |x, y| forest(x, y)).mul(&sil);
        ...
        let mut b = Held::new(Tool::round_sable(1.1), 234);
        let mut x = 0.0;
        while x < w {
            x += r.range(1.6, 3.4);
            let yc = near_crest(x);
            ...
            let tall = r.range(3.0, 7.5);
            let col = mix(ncol(x, yc + 3.0), hex("#2a3134"), 0.3, Mix::Pigment);
            b.load(pal.paint(col, 0.2), 0.5);
            let g = Gesture::new(vec![(x, yc + 2.0), (x + r.normal() * 0.2, yc - tall * 0.6), (x + r.normal() * 0.2, yc - tall)]).pressure(0.7, 0.1).ramps(0.0, 0.7).shake(0.3);
            c.drag(&mut b, &g, None);
        }
        c.dry();
    ...
    if o.stage("valley mist", &mut c, &mut rng) {
        ...
        let m = Stipple::new(Tool::stippler(2.6)).mixed(pal, 0.6).color(sea_col).coverage(move |x, y| 2.4 * sea(x, y)).pressure(0.5, 0.85).dips(18, 0.35, 0.6);
        c.stipple(&sm, &m, 253);
        c.dry();
    }
    ...
    if o.stage("heather", &mut c, &mut rng) {
        ...
        let hp = &hp;
        let hm = Mask::from_fn(f, move |x, y| smoothstep(0.6, 0.72, hp.get01(x, y * 1.8)) * smoothstep(4.0, 14.0, y - knoll(x))).mul(&knoll_m);
    ...
    let kn_cover = move |x: f32, y: f32| {
        ...
        band * away * smoothstep(0.52, 0.66, pine.get01(x, y * 1.8))
    };
    ...
    if o.stage("slope trees", &mut c, &mut rng) {
        // lower down the knoll's right flank, spruces going down into the
        // mist; the mist climbs the flank and swallows their feet
        ...
        let climb = move |x: f32, y: f32| {
            let d = y - knoll(x);
            let up = smoothstep(560.0, 780.0, x) * smoothstep(-10.0, 60.0, d);
            (up * sea(x, y.max(sea_top(x) + 5.0)).max(0.5) * (0.6 + 0.4 * billow.get01(x * 0.6, y * 2.0))).clamp(0.0, 1.0)
        };
        let cm = Mask::from_fn(f, move |x, y| if climb(x, y) > 0.03 { 1.0 } else { 0.0 });
        let m = Stipple::new(Tool::stippler(2.4)).mixed(pal, 0.65).color(sea_col).coverage(move |x, y| 2.6 * climb(x, y)).pressure(0.5, 0.85).dips(16, 0.35, 0.6).aim(false);
        c.stipple(&cm, &m, 420);
        c.dry();
    }
    ...
    if o.stage("grass", &mut c, &mut rng) {
        ...
        let mut b = Held::new(Tool::rigger(0.7), 362);
        let mut n = 0;
        let mut x = 0.0;
        while x < w {
            x += r.range(0.6, 2.2);
            ...
            for _ in 0..3 {
                ...
                let col = if r.chance(0.35 * near_top * near_top) { hex("#8a7d58") } else if r.chance(0.5) { mix(hex("#4c4630"), hex("#6d6440"), r.range(0.0, 1.0), Mix::Pigment) } else { hex("#15130f") };
                if n % 6 == 0 {
                ...
                c.drag(&mut b, &Gesture::new(vec![(x, y), midp, tip]).pressure(0.8, 0.05).ramps(0.0, 0.6).shake(0.5), None);
            }
        }
        c.dry();
    }
...
fn wanderer(c: &mut paint::Canvas, pal: &paint::Palette, (x, y): (f32, f32), s: f32, seed: u64) {
    ...
    let hair = pal.paint(hex("#3a2a1e"), 0.15);
