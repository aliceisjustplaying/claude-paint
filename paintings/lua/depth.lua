-- easel session "depth": a painting replayed chunk by chunk.
--   easel run paintings/lua/depth.lua [--width 3200]
...
-- shadows from the world's one sun: cast (soft at the far end, crisp at the
-- foot) and the contact seams, as transparent darks over the dry beach
...
-- a second sea: a stippled veil of smoother sea color, laid behind the man
-- and the stones (nothing to subtract by hand)
local function veil(x, y) return mix(shift(sk:at(x, 2 * HZ - y - 4), -0.04, 0, -0.01), "#3e4e5e", 0.3 + 0.45 * clamp((y - HZ) / 110, 0, 1)) end
stipple(seaband, {width=2.6, color=veil, coverage=3, pressure={0.4, 0.8}, medium=0.55, behind={"figure", "bodies"}})

--@ chunk 10 · clock 40741.9443359375
-- glints where the water is seen, never on the stones or the man: sparse,
...
work(zone, {hand="detail", tool="round 1", length={3, 11}, coverage=0.18, angle=0, order="scatter",
  color=function(x, y) return mix("#e6dcc0", "#aab0aa", clamp((y - HZ) / 70, 0, 1)) end, visible="water"})
