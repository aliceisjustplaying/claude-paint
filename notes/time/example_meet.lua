-- notes/time: paintings/lua/example.lua painted in three sittings with hand time on, each
-- passage laid only where it shows (the ridge stops just below the knoll's line).
--   easel run notes/time/example_meet.lua   (see notes/time.md)
-- Each "--@ chunk" line starts one chunk as it was run at the easel (clock = painting minutes).

--@ chunk 1 · clock 0
canvas{style="friedrich", aspect=1.4, seed=7, hand=true}; print(H); print(pal)
local t = timesheet(); print(string.format("TIME clock %.0f min · sitting %d at %.0f min · open %.0f%% setting %.0f%% tacky %.0f%% dry %.0f%% · %.0f min of hand time so far", t.clock, t.sittings, t.sitting, 100*t.open, 100*t.setting, 100*t.tacky, 100*t.dry, t.hand_min))
...
--@ chunk 3 · clock 0
-- sitting 2, the next morning: the distance, the knoll and the mist, wet into each other
rest(16)
far = noise{seed=3, octaves=5, period=260}
crest = function(x) return 420 - 45*far:at01(x, 0) - 25*math.exp(-((x-640)/120)^2) end
-- in one sitting the paint under a later passage is still open: lay the ridge only
-- where it shows, a little past the line the knoll will take
near = noise{seed=11, octaves=5, period=180}
knoll = function(x) return 560 - 70*math.exp(-((x-300)/260)^2) + 12*near(x, 0) end
ridge = (below(crest) - below(function(x) return knoll(x) + 10 end)):roughen(1.5, 12)
...
--@ chunk 6 · clock 0
-- sitting 3, the next day: the spruce and the grass over the set ground
rest(16)
