...
fn main() {
    ...
    let pal = &st.palette;
    let mut c = run.canvas(|| st.prepare(run.width, ASPECT, run.seed));
    let f = c.frame();
    let h = c.height();
