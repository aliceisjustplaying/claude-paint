...
/// `clip=` on a pass is true or false: a mask there counts as true and
/// clips to the pass's own region (old logs replay that way), so say so.
fn clip_note(st: &S, o: &Table, verb: &str) -> Result<()> {
    if let Value::UserData(u) = o.get::<Value>("clip")?
        && u.borrow::<M>().is_ok()
    {
        let mut s = st.borrow_mut();
        let note = format!("{verb}: clip= is true or false; the mask given was not used (the strokes clip to the region itself). For a looser or softer edge use edge= (easel guide, Edges)\n");
        if !s.out.contains(&note) {
            s.out.push_str(&note);
        }
    }
    Ok(())
