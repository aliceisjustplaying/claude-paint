...
## Setup
- Worktree: {WT} (branch {BRANCH}). Work only there. Do not push.
- Your painting: paintings/src/bin/{BIN}.rs. Run: `cargo paint {BIN}`
...
- Scratch: run `~/.local/bin/agent-tmp {BIN}` once and export
...
1. paintings/src/bin/{BIN}.rs and final renders out/{BIN}.png (1000px) and
   out/{BIN}_full.png (3200px).
2. notes/{BIN}.md, written continuously: the composition and why (what in
