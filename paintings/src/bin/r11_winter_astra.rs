...
use paint::{
    Canvas, Gesture, Held, Mask, Mix, Paint, Rgb, Rng, Shape, Stipple, Style, Tool, gradient, hex,
    smoothstep,
};
...
fn line(c: &mut Canvas, pts: &[Pt], width: f32, end: f32, p: Paint, seed: u64) {
    ...
    let tool = Tool {
        ...
        ragged: 0.16,
        run: 480.0,
        ..Tool::round_sable(width.max(0.22) * 1.32)
    };
...
fn stream(y: f32) -> (f32, f32) {
    ...
    (
        mid,
        2.0 + 92.0 * t.powf(1.55) + t * (2.1 * (y * 0.19).sin() + 0.9 * (y * 0.57).sin()),
    )
}
...
fn oak_boughs() -> Vec<(Vec<Pt>, f32)> {
    vec![
        ...
        (
            vec![(141., 406.), (160., 365.), (153., 328.), (166., 295.)],
            6.5,
        ),
        (
            vec![(131., 361.), (95., 349.), (65., 322.), (43., 319.)],
            5.,
        ),
        (
            vec![
        ...
        (
            vec![(306., 389.), (309., 350.), (329., 318.), (326., 287.)],
            6.5,
        ),
        (
            vec![(349., 376.), (382., 394.), (425., 386.), (455., 398.)],
            5.8,
        ),
        (
            vec![
        ...
        (
            vec![(203., 335.), (227., 300.), (222., 267.), (236., 230.)],
            5.5,
        ),
        (
            vec![
        ...
        (
            vec![(282., 300.), (314., 274.), (332., 242.), (356., 228.)],
            4.4,
        ),
...
fn twig(
    ...
    let q = (
        p.0 + angle.cos() * len * 0.53,
        p.1 + angle.sin() * len * 0.53,
    );
    let end = (
        q.0 + (angle + bend).cos() * len * 0.47,
        q.1 + (angle + bend).sin() * len * 0.47,
    );
    ...
    twig(
        ...
        angle + bend + r.range(-0.24, 0.23),
        len * r.range(0.52, 0.79),
        width * 0.65,
        depth - 1,
        ink,
        r,
    ...
    twig(
        c,
        q,
        angle + side * r.range(0.43, 0.91),
        len * r.range(0.38, 0.66),
        width * 0.53,
        depth - 1,
        ink,
        r,
...
fn oak(c: &mut Canvas, st: &Style, r: &mut Rng) {
    ...
    let gray = st.palette.paint(hex("#605e54"), 0.28);
    let light = st.palette.paint(hex("#777164"), 0.28);
    ...
    // Truncate the circular brush-width cap below the turf line, then grow
    // separate tapering buttresses into the bank. A ribbon's round end is not a root.
    let mut wood = Mask::from_shape(c.frame(), outline).mul_fn(|x, y| {
        let turf = 600. + 0.04 * (x - 215.) + 1.3 * (x * 0.63).sin() + 0.7 * (x * 1.17).sin();
        1. - smoothstep(turf - 1.1, turf + 0.7, y)
    });
    let roots = [
        vec![(221., 555.), (207., 584.), (186., 601.), (146., 612.)],
    ...
    for pts in &roots {
        ...
        let widths: Vec<_> = path
            .iter()
            .enumerate()
            .map(|(i, _)| 11.5 * (1. - i as f32 / (path.len() - 1) as f32).powf(1.3) + 0.12)
            .collect();
        wood = wood.union(&Mask::from_shape(
            c.frame(),
            Shape::new().ribbon(&path, &widths),
        ));
        for (i, pair) in path.windows(2).enumerate().step_by(3) {
            wood_axes.push((pair[0], pair[1], widths[i]));
        }
    ...
    let pal = st
        .palette
        .only(&["raw umber", "bone black", "yellow ochre", "lead white"]);
    let mut handling = st
        .body()
        .palette(&pal)
        .color(|x, y| {
    ...
    for (pts, w) in &boughs {
        ...
        if *w > 20.0 {
            // one ragged, dead leader, not a twig brush
            line(
                c,
                &[p, (p.0 - 1.4, p.1 - 9.0)],
                1.8,
            ...
            line(
                c,
                &[(p.0 - 3.0, p.1 + 3.0), (p.0 - 5.0, p.1 - 7.0)],
                1.3,
                0.2,
                dark,
                r.next_u64(),
    ...
    line(
        c,
        &[(219., 532.), (213., 540.), (215., 554.), (220., 559.)],
        2.2,
        0.3,
        dark,
        r.next_u64(),
    ...
    for _ in 0..46 {
        let y = r.range(474., 587.);
        let x = 218. + r.range(-6., 7.) + 3. * (y * 0.09).sin();
        line(
            c,
            &[(x, y), (x + r.range(-1.5, 1.5), y + r.range(2., 10.))],
            r.range(0.3, 0.8),
            0.12,
            dark,
            r.next_u64(),
...
fn fir(c: &mut Canvas, st: &Style, x: f32, base: f32, h: f32, col: Rgb, r: &mut Rng) {
    ...
    for i in 1..count {
        ...
        for s in [-1., 1.] {
            ...
            line(
                c,
                &[(x, y - 1.), (x + s * reach * 0.5, y + h * 0.025), end],
                (h * 0.062 * t).max(0.55),
                0.22,
                p,
                r.next_u64(),
...
fn main() {
    ...
    if o.stage("drawing", &mut c, &mut r) {
        ...
        for (pts, _) in oak_boughs() {
            let mark =
                paint::graphite::hand_line(&pts, &[0.18, 0.28], false, false, 0.12, r.next_u64());
            c.draw(&pencil, &mark, 0.05, r.next_u64());
        }
        let pts: Vec<_> = (0..36)
            .map(|i| {
                let y = 420. + i as f32 * 8.;
                (stream(y).0, y)
            })
            .collect();
    ...
    if o.stage("distance", &mut c, &mut r) {
        ...
        for (k, col) in ["#aaaeb1", "#929da5", "#7f8d97"].iter().enumerate() {
            let crest = f.per_column(move |x| {
                382. + k as f32 * 16.
                    + 13. * (x / 150. + k as f32 * 1.2).sin()
                    + 9. * n.get(x, 25. * k as f32)
            });
                ...
                &st.body()
                    .color(move |x, y| {
                        lerp(
                            col,
                            hex("#c4c3bc"),
                            smoothstep(395., 452., y) * 0.8 + 0.02 * n.get(x, y),
                            Mix::Pigment,
                        )
                    })
                    .coverage(3.0)
        ...
        for _ in 0..100 {
            let x = r.range(5., 995.);
            let y = 425. + 6. * (x / 63.).sin();
            let h = r.range(3., 15.) * (0.6 + 0.4 * ((x - 520.).abs() / 520.));
            fir(&mut c, &st, x, y, h, hex("#929c9e"), &mut r);
        }
        c.dry();
        let mist = Mask::from_fn(f, |_, y| {
            smoothstep(397., 420., y) * (1. - smoothstep(442., 460., y))
        });
        c.stipple(
            &mist,
            &Stipple::new(Tool::stippler(3.0))
                .mixed(&st.palette, 0.55)
                .color(|_, _| hex("#c7c7c0"))
                .coverage(|_, y| 1.8 * (-((y - 429.) / 20.).powi(2)).exp())
                .aim(false)
                .fade(1.6)
                .dips(24, 0.28, 0.6),
            225,
    ...
    if o.stage("snow and ice", &mut c, &mut r) {
        ...
        let ice = Mask::from_fn(f, move |x, y| {
            let (mid, w) = stream(y);
            smoothstep(410., 422., y)
                * (1. - smoothstep(w - 1.0, w + 1.2, (x - mid + 2. * n.get(x, y)).abs()))
        });
        let ice_pal = st
            .palette
            .only(&["lead white", "cobalt blue", "raw umber", "bone black"]);
        c.work(
            &ice,
            &st.body()
                .palette(&ice_pal)
                .color(move |x, y| {
                    ...
                    lerp(
                        hex("#bcc4c5"),
                        hex("#738591"),
                        0.22 + 0.65 * t + 0.06 * n.get(x * 0.5, y * 2.),
                        Mix::Pigment,
                    )
                })
                .coverage(4.)
        ...
        for side in [-1., 1.] {
            for j in 0..27 {
                ...
                let y = 457. + j as f32 * 9.;
                let (m, w) = stream(y);
                let (m2, w2) = stream(y + 8.);
                let x = m + side * w;
                ...
                if j % 3 == 0 {
                    line(
                        &mut c,
                        &[(x - side * 2., y - 1.5), (m2 + side * (w2 - 2.), y + 6.)],
                        1.0 + (y - 450.) * 0.007,
                        0.3,
                        lip,
                        r.next_u64(),
        ...
        for j in 0..31 {
            ...
            line(
                ...
                &[
                    (x, y),
                    (x + len * 0.6, y + r.range(-1., 1.)),
                    (x + len, y + 1.0),
                ],
                r.range(0.35, 0.9),
                0.15,
                if j % 3 == 0 { seam } else { frost },
                r.next_u64(),
            );
        }
        ...
        line(
            &mut c,
            &[(551., 680.), (573., 667.), (594., 665.), (607., 652.)],
            0.8,
            0.15,
            split,
            r.next_u64(),
        ...
        for (y, fraction, w) in [(646., 0.42, 13.), (675., 0.48, 19.), (609., 0.25, 7.)] {
            let (mid, half_width) = stream(y);
            let x = mid + fraction * half_width;
            line(
                &mut c,
                &[(x - w, y), (x, y - 1.), (x + w * 0.7, y + 1.3)],
                3.1,
                0.9,
                seam,
                r.next_u64(),
    ...
    if o.stage("snow relief", &mut c, &mut r) {
        ...
        c.work(
            &drifts,
            &st.broad()
                .color_over(|_, _, u| paint::shift(u, -0.017, 0., -0.005))
                .coverage(2.1)
                .length(24., 63.)
                .angle(|_, _| -0.08)
                .curve(0.065, 0.3)
        ...
        let hollow = Mask::from_fn(f, move |x, y| {
            let dx = (x - 217.) / 94.;
            let dy = (y - 604. - 0.04 * (x - 217.) - 2. * n.get(x, y)) / 10.;
            (-dx * dx * 1.8 - dy * dy * 1.6).exp()
        });
        c.work(
            &hollow,
            &st.body()
                .color_over(|_, _, u| paint::shift(u, -0.037, 0.0, -0.007))
                .coverage(2.3)
                .length(9., 25.)
                .angle(|_, _| 0.05)
                .medium(0.28)
    ...
    if o.stage("woods", &mut c, &mut r) {
        ...
        line(
            &mut c,
            &[(739., 472.), (735., 432.), (740., 400.), (736., 375.)],
            2.2,
            0.3,
            p,
            r.next_u64(),
    ...
    if o.stage("banks and traveler", &mut c, &mut r) {
        ...
        .enumerate()
        {
            ...
            let pp: Vec<_> = pts
                ...
                .collect();
            let rock = Mask::from_shape(f, Shape::new().poly(&pp)).roughen(
                800 + k as u32,
                7.,
                0.9 * s,
                0.3,
                ...
                &st.body()
                    .color(move |xx, yy| {
                        lerp(
                            hex("#565e5b"),
                            hex("#96998e"),
                            (0.32 - 0.017 * (yy - y) + 0.13 * n.get(xx, yy)).clamp(0., 1.),
                            Mix::Pigment,
                        )
                    })
                    .coverage(3.5)
            ...
            let pp: Vec<_> = top
                ...
                .collect();
            let cap = Mask::from_shape(f, Shape::new().poly(&pp))
                .roughen(820 + k as u32, 5., 1.1 * s, 0.6)
                .mul(&rock);
            c.work(
                &cap,
                &st.detail()
                    .color(|_, _| hex("#c4c9c1"))
                    .coverage(3.2)
                    .length(3., 10.)
                    .angle(|_, _| -0.2)
                    .medium(0.1)
            ...
            for _ in 0..28 {
                let xx = x + r.range(-25., 26.) * s;
                let yy = y + r.range(-7., 7.) * s;
        ...
        let shadow = st.palette.paint(hex("#a4adb0"), 0.38);
        line(
            &mut c,
            &[(613., 509.), (627., 510.), (648., 514.)],
            3.1,
            0.2,
            shadow,
            r.next_u64(),
        ...
        for j in 0..13 {
            let t = j as f32 / 12.;
            let y = 514. + t * t * 69.;
            let x = 627. + t * 41. + 4. * (t * 5.).sin();
            line(
                &mut c,
                &[
                    (x + if j % 2 == 0 { 2. } else { -2. }, y),
                    (x + 1., y + 1.5 + t),
                ],
                1.2 + t * 1.8,
                0.4,
    ...
    if o.stage("winter particulars", &mut c, &mut r) {
        ...
        for i in 0..360 {
            ...
            if i >= 95 && (tuft_field.get(x, y) < 0.08 || r.f() < 0.25) {
                continue;
            }
            ...
            for j in 0..(3 + (r.f() * 5.) as usize) {
                ...
                line(
                    &mut c,
                    &pts,
                    r.range(0.35, 0.85) * scale,
                    0.08,
                    if j % 4 == 0 { pale } else { grass },
                    r.next_u64(),
                );
                if j == 0 && i % 5 == 0 {
                    let top = pts[2];
                    for k in 0..3 {
                        dab(
                            &mut c,
                            (top.0 + k as f32 * 0.45, top.1 + k as f32 * 1.1),
                            0.8 * scale,
                            grass,
                            r.next_u64(),
                        );
        ...
        for j in 0..9 {
            let x = 701. + r.range(-14., 24.);
            let y = 677. + r.range(-5., 17.);
            let h = r.range(23., 45.);
            let lean = r.range(-11., 7.);
            let top = (x + lean, y - h);
            line(
                &mut c,
                &[(x, y), (x + lean * 0.4, y - h * 0.6), top],
                0.85,
                0.22,
                grass,
                r.next_u64(),
        ...
        for i in 0..42 {
            let x = r.range(3., 424.);
            let y = 628. + 0.10 * x + r.range(-10., 24.);
            if tuft_field.get(x, y) < 0.12 {
                continue;
            }
            let len = r.range(3., 17.);
            line(
                &mut c,
                &[(x, y), (x + len * 0.6, y - 1.), (x + len, y + 0.4)],
                r.range(0.7, 2.),
                0.2,
                soil,
                r.next_u64(),
        ...
        line(
            &mut c,
            &[(285., 664.), (310., 654.), (343., 652.), (361., 643.)],
            2.3,
            0.4,
            wood,
            r.next_u64(),
        ...
        for _ in 0..70 {
            let x = r.range(15., 950.);
            let y = r.range(439., 495.);
            let (m, w) = stream(y);
            if (x - m).abs() < w + 5. {
                continue;
            }
            line(
                &mut c,
                &[(x, y), (x + r.range(-1., 2.), y - r.range(1., 4.))],
                0.45,
                0.12,
                p,
                r.next_u64(),
