-- easel session "edges_new": a painting replayed chunk by chunk.
--   easel run paintings/lua/edges_new.lua [--width 3200]
...
--@ chunk 3 · clock 0
-- the new way: the same passes, the edges decided. The ridge is found where it turns against the glow,
-- soft along its flanks, lost where it runs down into the haze; the mirror's lower edge is lost in stretches
local sn = noise{seed=9, period=60}
ridgeQ = function(x, y)
  local glow = G(x, SUNX, 150)
  local low = smoothstep(HZ - 60, HZ - 10, crest(x))           -- the ridge runs low into the haze
  return clamp(0.5 - 0.5*glow + 0.5*low + 0.25*sn(x, y), 0, 1)
end
work(rangeM, {hand="body", tool="filbert 6", color=rangecol, angle=0.04, angle_jitter=0.12, length={24, 80}, coverage=3.0, medium=0.3, load=0.8, pal=skypal,
  edge=ridgeQ})
work(reflM, {hand="body", tool="filbert 6", color=function(x, y) return mix(rangecol(x, HZ - (y - HZ)/0.8), watercol(x, y), 0.3) end,
  angle=0, angle_jitter=0.01, curve={0, 0}, length={30, 110}, coverage=2.8, medium=0.3, load=0.85, pal=skypal,
  edge={soft=0.5, lost=0.5, period=50}})

--@ chunk 4 · clock 0
-- once it's dry, lose the mirror's lower edge in stretches: level strokes of the water dragged back across it
dry()
local lowerEdge = function(x, y) return y > HZ + 6 and 0.7 or 0 end
print(lose(reflM, {where=lowerEdge, tool="filbert 6", angle=0, reach={6, 8}, mix=0.5, load=0.3, pressure={0.75, 0.25}, medium=0.6}))
