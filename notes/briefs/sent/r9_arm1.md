...
## Your medium: the easel, without the procedural generators
Paint live at the Lua easel (crates/easel/README.md is its guide). In
this setup the tools that grow or compute a motif or a scene for you are
taken away: **always** `export EASEL_WITHOUT=procedural` in every shell
before any `easel` command. Write your own motifs as your hand would make
them (your own trees, figures, rocks: functions in Lua, drawn stroke by
stroke, looking as you go). Leave hand time off (no `hand=true`). Your
source is the session log: `paintings/lua/<name>.lua`.

## Setup
- Worktree: ~/src/a/claude-paint-r9-arm1 (branch r9-arm1). Work only there. Do not push. Commit
...
- Scratch: run `~/.local/bin/agent-tmp r9arm1` once and export
...
1. Your painting's source (see your medium) and plain renders copied to
   notes/round9/arm1/painting_1000.png and painting_3200.png (lossless).
2. notes/round9/arm1/notes.md, written continuously: the composition and
