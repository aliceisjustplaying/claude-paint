...
-- mix piles, don't paint a formula: seven piles knifed along the sky's range, looking at the dead color
-- they go over; each stroke loads from one, the steps are made on the canvas (wet, then the blend)
SKYP = piles(skyB, {n=7, over=skyS, pal=skypal, medium=0.3, coverage=3.8, load=0.75, seed=601})
print(SKYP)
work(skyS, {hand="broad", color=SKYP, angle=0, angle_jitter=0.02, length={120, 320}, coverage=3.8, medium=0.3, load=0.75,
...
BANKP = piles(cloudcol, {n=4, over=cm, pal=skypal, medium=0.32, coverage=2.2, load=0.9, seed=701})
work(cm, {hand="broad", color=BANKP, hug=false, angle=-0.03, angle_jitter=0.02, curve={0, 0}, length={90, 260},
...
LITP = piles(function(x, y) return mix("#bfa99c", "#e2c9a0", G(x, SUNX, 200)) end, {n=3, over=lightm, pal=skypal, medium=0.3, coverage=1.2, load=0.9, seed=702})
work(lightm, {hand="broad", color=LITP,
...
WATERP = piles(waterCol, {n=6, over=openW, pal=skypal, medium=0.3, coverage=3.8, load=0.75, seed=801})
print(WATERP)
work(openW, {hand="broad", color=WATERP, angle=0, angle_jitter=0.004, curve={0, 0}, length={120, 320},
...
work(fixW, {hand="body", tool="filbert 5", color=WATERP, angle=0, angle_jitter=0.004, curve={0, 0}, length={30, 90},
